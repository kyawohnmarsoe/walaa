...
export default function EditForm({ className = '', ticket, customers, updated_by_loggedin_user, remarks, users, issues }) {
    ...
    function handleSubmit(e) {
        ...
        router.post(`/device/tickets/${ticket.id}`, values)
    }
    ...
    return (
        <section className={className}>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-6 ">
                ...
                {
                    remarks && remarks.map((rm, index) => (
                        <>
                            <div key={index} className="max-w-xl rounded overflow-hidden shadow-lg px-3 pt-4 mb-4">
                                <div className="grid grid-cols-3 gap-4">
                                    <div className="col-span-2">
                                        {rm.remarks}
                                    </div>
                                    <div className="text-right">
                                        <a href={`/device/tickets/delete_remark/${rm.id}`} className="text-sm text-red-500 underline remove_rm">Remove</a>
                                    </div>
                                </div>

                                <div className="flex items-center pb-3">
                                    <div className="mt-4 mb-2">
                                        <p key={`prm_${rm.id}`} className="text-sm text-gray-900 leading-none mb-1">
